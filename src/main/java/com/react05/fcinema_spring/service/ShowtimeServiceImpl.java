...
public class ShowtimeServiceImpl implements ShowtimeService {
    ...
    public ApiResponse<PageResponse<ShowtimeResponse>> getShowtimesWithFilters(
        ...
        Page<Showtime> showtimePage =
                showtimeRepository.findWithFilters(
        ...
        PageResponse<ShowtimeResponse> pageResponse =
                PageResponse.<ShowtimeResponse>builder()
                        .content(content)
        ...
        return ApiResponse.<PageResponse<ShowtimeResponse>>builder().result(pageResponse).build();
    }
    ...
    public ApiResponse<List<ShowtimeResponse>> getShowtimesByCinemaRoom(Integer roomId) {
        CinemaRoom room =
                cinemaRoomRepository
                        .findById(roomId)
                        .orElseThrow(() -> new AppException(ErrorCode.ROOM_NOT_FOUND));
        List<Showtime> showtimes = showtimeRepository.findByCinemaRoom(room);
        return ApiResponse.<List<ShowtimeResponse>>builder()
                .result(showtimeMapper.toResponseList(showtimes))
                .build();
    }
    ...
    @Override
    @Transactional
    public ApiResponse<ShowtimeResponse> updateShowtime(Integer id, ShowtimeUpdateRequest request) {
        ...
        if (request.getShowDateTime() != null || request.getEndDateTime() != null) {
            LocalDateTime showStartTime =
                    request.getShowDateTime() != null
                            ? request.getShowDateTime()
                            : showtime.getShowDateTime();
            LocalDateTime showEndTime =
                    request.getEndDateTime() != null ? request.getEndDateTime() : showtime.getEndDateTime();
            ...
            List<Showtime> conflicts =
                    showtimeRepository.findConflictingShowtimes(
                            showtime.getCinemaRoom().getId(), showStartTime, showEndTime);
        ...
        if (request.getStatus() != null) {
            showtime.setStatus(getShowtimeStatus(request.getStatus()));
        }
    ...
    private void validateShowtimeDates(
            LocalDateTime showStartTime, LocalDateTime showEndTime, Movie movie) {
    ...
    private void validateNoConflicts(
            CinemaRoom room, LocalDateTime startTime, LocalDateTime endTime) {
        List<Showtime> conflicts =
                showtimeRepository.findConflictingShowtimes(room.getId(), startTime, endTime);
        if (!conflicts.isEmpty()) {
            throw new AppException(ErrorCode.SHOWTIME_CONFLICT);
        }
    }

    private Showtime.Status getShowtimeStatus(String status) {
        try {
            return Showtime.Status.valueOf(status.toUpperCase());
        } catch (IllegalArgumentException e) {
            throw new AppException(ErrorCode.INVALID_STATUS);
        }
    }
